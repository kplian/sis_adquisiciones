               ...
               execute(v_consulta) into v_year_ini,v_month_ini,v_day_ini ;
                ...
                IF v_envio = true THEN
                    ...
                    IF v_record.fechaaccion is not null THEN
                        v_record.fechaaccion = 'EL '||v_record.fechaaccion::VARCHAR;
                    END IF;

                    v_titulo = 'BOLETA POR VENCER-GESTOR '||COALESCE(v_record.gestor,'')||'- INICIO DE PROCESO DE CIERRE '||COALESCE(v_record.fechaaccion::VARCHAR,'')||'';

...
BEGIN
    ...
    select c.*,s.id_funcionario,s.id_funcionario_aprobador, s.id_funcionario_rpc,s.fecha_inicio,s.justificacion
    into   v_cotizacion
    from adq.vcotizacion c
    inner join adq.tsolicitud s on s.id_solicitud = c.id_solicitud
    where id_cotizacion = p_id_cotizacion;

    --chros 16/11/2018 - adquisiciones solo dispara contratos administrativos
    ...
    */
    ...
    v_solicitud = 'Por favor realizar contrato para la solicitud de compra con numero de tramite : ' || v_cotizacion.num_tramite || '. Categoria : ' || v_cotizacion.nombre_categoria || '. Concepto : ' || v_cotizacion.tipo_concepto;

    /*Obtener el estado de registro*/
    --chros 16/11/2018 - aparentemente no se usa en ningún lado
    /*
    select te.id_tipo_estado
    into v_id_tipo_estado_registro
    from wf.ttipo_estado te
    inner join wf.ttipo_proceso tp on te.id_tipo_proceso = tp.id_tipo_proceso
    inner join wf.tproceso_wf p on p.id_tipo_proceso = tp.id_tipo_proceso
    ...
    */
    ...
    IF v_num_estados = 1 then
        ...
        IF v_num_deptos = 1 THEN
            -- si solo es un depto, recuperamos el depto correspondiente
            SELECT id_depto
            into v_id_depto_estado
            FROM wf.f_depto_wf_sel(
                    p_id_usuario,
                    va_id_tipo_estado[1],
                    now()::date,

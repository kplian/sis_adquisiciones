...
(inserciones, modificaciones, eliminaciones de la tabla 'adq.tdocumento_sol'
...
|| ''',''' ||
                         ...
                         coalesce (v_parametros.doc_id::text, 'NULL') || ',''' ||  v_parametros.nombre_institucion || ''',''' || v_parametros.direccion_institucion || ''',' ||
                          coalesce (v_parametros.casilla::text, 'NULL') 
|| ',' ||   coalesce (v_parametros.telefono1_institucion::text, 'NULL') 
|| ',' || coalesce (v_parametros.telefono2_institucion::text, 'NULL') ||
',' ||
                          coalesce
(v_parametros.celular1_institucion::text, 'NULL') || ',' || coalesce 
(v_parametros.celular2_institucion::text, 'NULL') || ',' ||   coalesce 
(v_parametros.fax::text, 'NULL') || ',''' ||
...
|| ''',
                         ''' ||  coalesce (v_parametros.nit::text,
'NULL') ||''',' ||  coalesce (v_parametros.id_lugar::text, 'NULL')|| ',''' ||  v_parametros.register || ''',''' ||
                         ...
                         coalesce (v_parametros.telefono1::text, 'NULL') 
|| ''',''' || coalesce (v_parametros.telefono2::text, 'NULL') || ''',''' 
||  v_parametros.genero || ''',' ||  coalesce ('''' ||
v_parametros.fecha_nacimiento::text || '''', 'NULL') || ',''' ||
                 ...
                 v_consulta,TRUE)AS t1(resp varchar)
                  ...
                  select * FROM dblink(migra.f_obtener_cadena_conexion(),
                  'SELECT * FROM migracion.f_sincronizacion()',FALSE)AS
t1(resp varchar)
                  into v_resp;
  ...
  v_parametros.id_persona,    case when v_parametros.id_persona is NULL THEN
'institucion'
else
'persona'
end,
                     v_parametros.numero_sigma,  v_parametros.codigo,   
      v_parametros.nit,
                     v_parametros.id_lugar,
  v_parametros.rotulo_comercial)RETURNING id_proveedor into v_id_proveedor;
                 else
  ...
  tipo,
                  ...
                  nit,
                         ...
                         (p_id_usuario,                    now(),     
                  'activo',
                         v_id_institucion,  v_id_persona,               
  case when v_id_persona is NULL THEN
'institucion'
else
'persona'
end,
                         v_parametros.numero_sigma, 
  v_parametros.codigo,        v_parametros.nit,
                         v_parametros.id_lugar,
  v_parametros.rotulo_comercial)RETURNING id_proveedor into v_id_proveedor;
                 end if;
             end if;
             ...
             v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Proveedor
almacenado(a) con exito');
             v_resp =
pxp.f_agrega_clave(v_resp,'id_proveedor',v_id_proveedor::varchar);
     ...
     ***********************************/
         ...
         begin
             if (pxp.f_get_variable_global('sincronizar') = 'true') then
                 ...
                 /*select * FROM dblink(migra.f_obtener_cadena_conexion(),
                 'SELECT *
                   FROM compro.f_tad_nuevo_proveedor_pxp_iud(''update'',
'|| v_parametros.id_proveedor || ', '|| coalesce (v_parametros.id_persona::text, 'NULL') || '
                   , ''' ||  v_parametros.codigo || ''', ''' || v_parametros.numero_sigma || ''', NULL,  '|| coalesce (v_parametros.id_institucion::text, 'NULL') || ', NULL,
                    NULL, NULL, NULL,
                     NULL, NULL, NULL,
                      NULL, NULL, NULL, NULL,
                       NULL, NULL, NULL, NULL, ''' || v_parametros.nit 
|| ''',
                         '|| coalesce (v_parametros.id_lugar::text,
'NULL') || ', NULL, NULL, NULL, NULL,
                         NULL, NULL, NULL, NULL, NULL, NULL, NULL
                         , NULL, NULL)',TRUE)AS t1(resp varchar)
                         into v_resp;*/
...
'NULL') ||''','  ||  coalesce (v_parametros.id_lugar::text, 'NULL')|| ',''' ||  v_parametros.register || ''',''' ||
                         ...
                         coalesce (v_parametros.telefono1::text, 'NULL') 
|| ',' || coalesce (v_parametros.telefono2::text, 'NULL') || ',''' ||
v_parametros.genero || ''',' ||  coalesce ('''' || v_parametros.fecha_nacimiento::text || '''', 'NULL') || ',''' ||
                         v_parametros.direccion || ''',''' || v_parametros.rotulo_comercial || ''')',TRUE)AS t1(resp varchar)
                         into v_resp;
...
modificado(a)');
             v_resp =
pxp.f_agrega_clave(v_resp,'id_proveedor',v_parametros.id_proveedor::varchar);
     ...
     ***********************************/
         ...
         begin
             ...
             if (pxp.f_get_variable_global('sincronizar') = 'true') then
                 ...
                 select * FROM dblink(migra.f_obtener_cadena_conexion(),
                 'SELECT *
                   FROM compro.f_tad_nuevo_proveedor_pxp_iud(''delete'',
'|| v_parametros.id_proveedor || ',NULL
                   , NULL, NULL, NULL,NULL, NULL,
                    NULL, NULL, NULL,
                     NULL, NULL, NULL,
                     NULL, NULL, NULL, NULL,

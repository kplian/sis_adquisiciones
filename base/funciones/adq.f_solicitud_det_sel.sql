...
BEGIN
	...
	***********************************/
    ...
    /*********************************    
 	#TRANSACCION:  'ADQ_SOLDREP_SEL'
 	#DESCRIPCION:	Consulta de datos para reporte de solicitude de compra
 	#AUTOR:		rac	
 	#FECHA:		016/10/2017
	***********************************/

	elseif(p_transaccion='ADQ_SOLDREP_SEL')then
       ...
       begin
			...
			v_consulta:='select
                            ...
                            tcc.control_partida,
                            CASE 
                               WHEN tcc.control_partida = ''no'' THEN
                                   0
                               ELSE
                                  sold.id_partida
                               END  as id_partida_control,
                           
                           CASE 
                               WHEN tcc.control_partida = ''no'' THEN
                                   ''No considera partida''::varchar
                               ELSE
                                  (par.codigo||'' ''||par.nombre_partida)::varchar
                               END  as desc_control_partida
                     
            			from adq.tsolicitud_det sold
                        join param.tconcepto_ingas cig on cig.id_concepto_ingas = sold.id_concepto_ingas
						join segu.tusuario usu1 on usu1.id_usuario = sold.id_usuario_reg
                        ...
                        join pre.tpartida par on par.id_partida = sold.id_partida                       
                        join conta.tcuenta cta on cta.id_cuenta = sold.id_cuenta
                        left join  conta.tauxiliar aux on aux.id_auxiliar = sold.id_auxiliar
